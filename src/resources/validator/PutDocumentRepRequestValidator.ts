...
export default class PutDocumentRepRequestValidator implements ExpressMiddlewareInterface {
    async use (request: Request, response: Response, next: NextFunction): Promise<void> {
        ...
        // userId.indexが文字列以外の場合
        if (this.isNull(requestBody['userId']['index'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(message.STRING_INVALID, 'userId.index'), 400);
        }
        // userId.valueが文字列以外の場合
        if (this.isNull(requestBody['userId']['value'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(message.STRING_INVALID, 'userId.value'), 400);
        }
        // id.indexが文字列以外の場合
        ...
        if (this.isNull(requestBody['id']['value'])) {
            // エラーレスポンスを返す
            throw new AppError(sprintf(message.STRING_INVALID, 'id.value'), 400);
        }
        // code.indexが文字列以外の場合
        ...
        await transformAndValidate(PutDocumentByUserIdReqDto, requestBody);
